...
@export var hover_dist := 40.0
@export var spring_str := 200.0
@export var damp_str := 20.0
@export var foot_spread := 16.0
@export var hip_y_offset := 18.0
@export var bounce_amp := 2.0

@export var step_trigger := 12.0
@export var step_duration := 0.11
@export var step_arc_h := 10.0
@export var look_ahead := 0.11

@export var air_foot_tuck_x := 14.0
@export var air_foot_tuck_y := 10.0

@export var stride_min_interval := 0.08
@export var stride_cycle_base := 0.24
@export var speed_step_boost := 0.55
@export var fast_step_duration_scale := 0.70
...
var _step_clock := 0.0
var _gait_timer := 0.0
var _last_stepped := 1 # 0 left, 1 right
...
func _ready() -> void:
	...
	_step_dur_l = step_duration
	_step_dur_r = step_duration
	_step_arc_l = step_arc_h
	_step_arc_r = step_arc_h
...
func _update_steps(delta: float, hip: Vector2, hit_l: bool, hit_r: bool, floor_y: float) -> void:
	...
	if prefer_left:
		if (dl > step_trigger or force_l) and l_ready:
			_begin_step(true, hip, floor_l, hit_l, speed_norm); _gait_timer = 0.0
		elif (dr > step_trigger or force_r) and r_ready:
			_begin_step(false, hip, floor_r, hit_r, speed_norm); _gait_timer = 0.0
	else:
...
func _begin_step(is_left: bool, hip: Vector2, floor_y: float, has_ground: bool, speed_norm: float) -> void:
	...
	if is_left:
		_step_from_l = foot_pos_l; _step_to_l = tgt; _step_t_l = 0.0
		_step_dur_l = dur; _step_arc_l = arc
		_last_stepped = 0; _last_step_time_l = _step_clock
	else:
		_step_from_r = foot_pos_r; _step_to_r = tgt; _step_t_r = 0.0
		_step_dur_r = dur; _step_arc_r = arc
		_last_stepped = 1; _last_step_time_r = _step_clock
