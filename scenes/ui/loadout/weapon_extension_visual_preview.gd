extends Control
class_name WeaponExtensionVisualPreview

const DEFAULT_PREVIEW_COLOR: Color = Color(0.8, 0.9, 1.0, 1.0)

var fallback_color: Color = DEFAULT_PREVIEW_COLOR
...
func set_extension(extension_variant: Variant) -> bool:
	_clear_visual()

	var definition: WeaponExtensionDefinition = _get_definition(extension_variant)
	if definition == null:
		return false
	fallback_color = definition.icon_color
...
func _get_definition(extension_variant: Variant) -> WeaponExtensionDefinition:
	...
	return extension_variant as WeaponExtensionDefinition
